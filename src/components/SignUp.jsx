...
function SignUp(props) {
    ...
    const validateFields = () => {
        ...
        if(password!==repassword){
            props.setToast({header:'Erro!',body:'As senhas não são as mesmas!'})
        ...
        if(password.length < 6){
            props.setToast({header:'Erro!',body:'A senha deve ter tamanho maior que 6'})
            props.setShowToast(true)
            setLoading(false)
            setValidate({login:'',password:'is-invalid',repassword:'is-invalid'})
            validateAccess = false
        ...
        FirebaseUserService.login(
            ...
            (user) => {
                if(user.email === login){
                    props.setToast({header:'Erro!',body:'Conta já cadastrada'})
                    props.setShowToast(true)
                    setLoading(false)
                    setValidate({login:'is-invalid',password:'',repassword:''})
                    validateAccess = false
                }
            }
        )
